...
## 2020.2.1
1. 增加了网格线和吸附到网格线功能。可以在改变画布背景颜色的面板里开启。
2. 增加了新的插件环境支持，可以使用js或者ts编写插件。

## 2020.2.0
1. 增加了编辑时间曲线的功能。在属性控制的过渡效果设置里，或者在动效编辑的过渡效果设置里，将“缓动函数”设置为Custom，然后就可以点击“编辑曲线”的按钮，通过设计一条曲线，实现自定义缓动曲线。（Unity SDK需要更新到仓库最新代码，其他SDK后续支持）。参考[自定义缓动曲线](../guide/editor/transition.html#自定义缓动曲线)。
2. 修正了使用格式为“16位/通道”的图片制作序列帧动画时，动画显示错误的问题。
3. 修正了标签组件、按钮组件属性界面里字体大小不能下拉选择的问题。

p1修正
1. 优化了时间轴多帧复制粘贴功能。

p2修正
1. 修正动效的第一帧是播放多个嵌套动效时，只有一个嵌套动效能播放的bug。
2. 修正同时使用“文本控制”和“多语言”时，预览报异常的bug。

p3修正
1. 修正插件运行时出现异常不显示在控制台的问题。

## 2020.1.2
1. 替换资源对话框增加了历史记录。
2. 修改行为：当插件在发布过程中抛出异常，将中断发布，而不是继续发布。
3. 修正了TextMeshPro文字在Linear颜色空间下颜色不正确的bug。
4. 修正了发布出来的纹理集有色差的bug。
5. 修正了发布出来的纹理集在win平台和mac平台不一致的bug。
6. 修正了如果纹理集只包含一张图片，大小限制为4的倍数无效的bug。
7. 修正了显示列表视图中，用shift选中多个元件这一功能不正确的bug。
8. 修正了用windows键+空格切换输入法后，无法正常输入文字或者有类似ctrl键被锁定的bug。
9. 修正了导入文件夹时软件闪退的bug。

p1修正
10. 修正了大量图片设置了特殊的BlendMode后，界面卡顿的bug。
11. 修正了批量修改图片属性后，文档没有刷新的问题。
12. 修正了分支发布的一个小问题，当某个分支下的资源标记为导出时，发布其他分支不应该包含这个资源。

## 2020.1.1
1. 增加了ThreeJS项目类型。[源码](https://github.com/fairygui/FairyGUI-threejs) [快速上手](https://github.com/fairygui/FairyGUI-threejs-example)
...
11. 增加了一个发布设置，当选择“主干包含所有分支”的发布方式时，可以选择是否将分支的图片发布到单独的纹理集。
  
[详情](https://mp.weixin.qq.com/s/8MKyJu-eoUCm7sOh7QredQ)
